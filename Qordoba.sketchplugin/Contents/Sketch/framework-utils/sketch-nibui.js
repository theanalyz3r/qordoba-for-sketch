 ...
 */
 /*
 * Copyright 2016 Qordoba
 *
 * Modyfications by Konrad Rodzik
 *
 */

function QordobaNibUI(frameworkBundle, nibName, bindViewNames) {
  return new NibUI(frameworkBundle, nibName, bindViewNames);
}

function QordobaNibUI2(context, bundleResourceName, nibName, bindViewNames) {
  var bundlePath = context.plugin.urlForResourceNamed(bundleResourceName).path();
  var bundle = NSBundle.bundleWithPath(bundlePath);
  return new NibUI(bundle, nibName, bindViewNames);
}

function NibUI(bundle, nibName, bindViewNames) {
